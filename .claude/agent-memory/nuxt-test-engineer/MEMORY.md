...
## Pro Feature Coverage (completed — RF-800 to RF-809): 216 tests
store 44, usePro 60, ProBanner 22, PricingTable 30, ProUpgradeModal 26, DonationForm 34.
Composable mock: `vi.mock('../composables/usePro', ...)`. See patterns.md §9+ for details.

## Admin Feature Coverage (completed — RF-1000 to RF-1009): 327 tests
- `admin.store.ts`: 75 tests — all actions (setStats, setUsers, updateUser, removeUser, setSelectedUser, clearSelectedUser, setShelters, updateShelter, removeShelter, setPetshops, updatePetshop, removePetshop, setAdminClinics, updateAdminClinic, removeAdminClinic, setTransactions, setLoading, clearAdmin) + hasStats/hasUsers getters
- `useAdmin.ts`: 76 tests — all fetch functions (stats/users/shelters/petshops/clinics/transactions), update/delete functions with ID validation guards, error extraction (4 shapes)
- `AdminDashboard.vue`: 27 tests — skeleton (8 cards), 8 KPI labels, revenue section, 5 quick-nav links, empty state + Reintentar button, error alert
- `AdminUserManager.vue`: 31 tests — skeleton, empty state, user rows, PRO/Admin badges, toggle PRO/Admin, 2-step delete (6 tests), result count, error alert
- `AdminShelterManager.vue`: 27 tests — same pattern + Verificado/Destacado badges, shelter contact info
- `AdminStoreManager.vue`: 27 tests — same pattern as ShelterManager for petshops
- `AdminClinicManager.vue`: 29 tests — same pattern + specialty chips (max 2 visible, +N overflow, dash for empty)
- `AdminTransactionLog.vue`: 28 tests — type badges (subscription=bg-primary, donation=bg-success), status badges (4 statuses), read-only (no delete/update buttons), truncated ID
- `admin.ts` middleware: 7 tests — 3 branches: unauthenticated→/login, auth+non-admin→/, auth+admin→undefined
- Composable mock path from components: `vi.mock('../composables/useAdmin', ...)`
- isAdmin requires `currentUser: { is_admin: true }` in initialState (computed from currentUser.is_admin)
- Plural form trap: AdminTransactionLog renders 'transacciónes' (with accent) — use `toContain('transacción')` not `toContain('transacciones')`
...
## Maintenance Feature Coverage (completed — RF-1200 to RF-1209): 163 tests
- `maintenance.store.ts`: 32 tests — initial state (status/isLoading/isEnabled/hasStatus), setStatus (full/minimal/optional fields/replace), setLoading (true/false/no side effects), isEnabled getter (null/true/false/reactive updates), hasStatus getter (null/after setStatus/after clear), clearMaintenance (all fields from populated state/safe on initial state), integration lifecycle
- `useMaintenance.ts`: 37 tests — fetchStatus (envelope shape/direct shape/loading lifecycle/SILENT error — error.value stays null/preserves existing status on retry failure/clears error at start), toggleMaintenance (envelope shape/direct shape/PUT call shape with true/false/loading lifecycle/error extraction: 3 shapes + generic fallback + null/no store update on failure), error ref contract (starts null/cleared by fetchStatus/cleared by toggleMaintenance success/cleared on next call after toggle error)
- `MaintenancePage.vue`: 18 tests — default render (role="main"/aria-label/single h1/h1 text/default subtitle/CTA href="/"/CTA text/CTA aria-label), accessibility (aria-hidden icon/CTA aria-label/single h1/main landmark label), with message prop (custom text/no default subtitle/h1 intact/CTA intact/long message), undefined message fallback
- `MaintenanceToggle.vue`: 52 tests — onMounted (calls fetchStatus once/with no args), loading skeleton (aria-busy/aria-label/no action btn/no empty state), empty state (text/Reintentar btn/Reintentar calls fetchStatus/no skeleton/no action btn), inactive state (bg-success badge/Inactivo text/aria-label/btn-danger action/Activar text/no confirm row/no empty state), active state (bg-danger badge/Activo text/aria-label/btn-success action/Desactivar text/no confirm row), two-step enable (confirm row shows/warns users/Confirmar+Cancelar visible/action btn hidden/Cancelar hides row/Cancelar no API call/Cancelar restores action btn/Confirmar calls true/once/btn-danger confirm), two-step disable (deactivation text/Confirmar calls false/btn-success confirm/aria-label group/Cancelar no API call), error display (alert role="alert"/message text/no alert when null), metadata (updated_by text/updated_at without updated_by/no metadata when absent/message in role="note"/note aria-label/no note when absent), loading during toggle (disabled btn/spinner/spinner role+aria-hidden/no spinner when false)
- `maintenance.ts` middleware: 24 tests — admin bypass (enabled: no redirect/undefined/on /maintenance/undefined/to //undefined, disabled: no redirect/undefined), non-admin maintenance enabled (redirect /→/maintenance/return signal/redirect /dashboard/return signal/redirect /blog/no redirect on /maintenance/undefined on /maintenance), non-admin maintenance disabled (no redirect //undefined/no redirect /dashboard/redirect stale /maintenance→//return { path: '/' }), unauthenticated+enabled (redirect /→/maintenance/return signal for /login/no redirect already on /maintenance), null status (no redirect — defaults maintenance OFF/undefined)

## isEnabled/isAdmin via createTestingPinia initialState for middleware
`maintenanceStore.isEnabled` is a `computed` from `status.is_enabled`. In middleware tests, set `maintenance: { status: { is_enabled: true } }` in initialState — NOT `maintenance: { isEnabled: true }` (computed properties are ignored in initialState).
Similarly, `authStore.isAdmin` is computed from `currentUser?.is_admin` — set `auth: { currentUser: { is_admin: true } }`.
