<script setup lang="ts">
// ShelterCard — compact card for a single shelter in the directory.
// Shows: logo (with fallback emoji), organization_name, verified badge,
// city, description excerpt, and link to /shelters/:id.

import type { Shelter } from '../types'

const props = defineProps<{
  shelter: Shelter
}>()

// ── URL safety guard ──────────────────────────────────────
function isSafeImageUrl(url: string | undefined): boolean {
  if (!url) return false
  if (url.startsWith('blob:')) return true
  if (url.startsWith('/') || url.startsWith('./') || url.startsWith('../')) return true
  try {
    const parsed = new URL(url)
    return parsed.protocol === 'https:' || parsed.protocol === 'http:'
  }
  catch {
    return false
  }
}

const safeLogoUrl = computed(() =>
  isSafeImageUrl(props.shelter.logo_url) ? props.shelter.logo_url : null,
)

const imgError = ref(false)
const showLogo = computed(() => !!safeLogoUrl.value && !imgError.value)

function onImgError(): void {
  imgError.value = true
}

watch(() => props.shelter.logo_url, () => {
  imgError.value = false
})

// ── Description excerpt (2 lines) ────────────────────────
const descriptionExcerpt = computed(() => {
  const desc = props.shelter.description
  if (!desc) return null
  return desc.length > 120 ? `${desc.slice(0, 120)}...` : desc
})
</script>

<template>
  <article
    class="card h-100 border-0 shadow-sm shelter-card"
    :aria-label="`Refugio ${shelter.organization_name}`"
  >
    <!-- Logo / fallback header -->
    <div class="shelter-card__photo-wrap">
      <img
        v-if="showLogo"
        :src="safeLogoUrl!"
        :alt="`Logo de ${shelter.organization_name}`"
        class="shelter-card__photo"
        width="400"
        height="200"
        @error="onImgError"
      />
      <div
        v-else
        class="shelter-card__fallback d-flex align-items-center justify-content-center"
        aria-hidden="true"
      >
        <span class="shelter-card__fallback-icon">🏠</span>
      </div>

      <!-- Badges overlaid on photo -->
      <div class="shelter-card__badges">
        <span
          v-if="shelter.verified"
          class="badge bg-success shelter-card__badge"
          aria-label="Refugio verificado"
        >
          Verificado
        </span>
      </div>
    </div>

    <div class="card-body d-flex flex-column gap-2 p-3">
      <!-- Name -->
      <h3 class="h6 fw-bold mb-0 shelter-card__name">{{ shelter.organization_name }}</h3>

      <!-- City + country -->
      <p class="text-muted small mb-0 shelter-card__location">
        <span aria-hidden="true">📍</span>
        {{ shelter.city?.name }}, {{ shelter.country?.name }}
      </p>

      <!-- Description excerpt -->
      <p v-if="descriptionExcerpt" class="text-muted small mb-0 shelter-card__desc">
        {{ descriptionExcerpt }}
      </p>
    </div>

    <!-- CTA footer with stretched-link so the whole card is clickable -->
    <div class="card-footer bg-transparent border-top-0 px-3 pb-3 pt-0">
      <NuxtLink
        :to="`/shelters/${shelter.id}`"
        class="btn btn-primary btn-sm w-100 stretched-link"
        :aria-label="`Ver refugio ${shelter.organization_name}`"
      >
        Ver refugio
      </NuxtLink>
    </div>
  </article>
</template>

<style scoped lang="scss">
.shelter-card {
  border-radius: var(--bs-border-radius-lg);
  transition: transform 0.15s ease, box-shadow 0.15s ease;

  &:hover {
    transform: translateY(-2px);
    box-shadow: 0 0.5rem 1.5rem rgba(0, 0, 0, 0.12) !important;
  }

  &__photo-wrap {
    position: relative;
    overflow: hidden;
    border-radius: var(--bs-border-radius-lg) var(--bs-border-radius-lg) 0 0;
    height: 160px;
    background-color: var(--bs-secondary-bg);
  }

  &__photo {
    width: 100%;
    height: 100%;
    object-fit: cover;
    display: block;
  }

  &__fallback {
    width: 100%;
    height: 100%;
    background-color: #dff0e8;
  }

  &__fallback-icon {
    font-size: 3rem;
    line-height: 1;
  }

  &__badges {
    position: absolute;
    top: 0.5rem;
    left: 0.5rem;
    display: flex;
    flex-direction: column;
    gap: 0.25rem;
  }

  &__badge {
    font-size: 0.7rem;
    letter-spacing: 0.03em;
  }

  &__name {
    display: -webkit-box;
    -webkit-line-clamp: 2;
    -webkit-box-orient: vertical;
    overflow: hidden;
  }

  &__location {
    overflow: hidden;
    text-overflow: ellipsis;
    white-space: nowrap;
  }

  &__desc {
    display: -webkit-box;
    -webkit-line-clamp: 2;
    -webkit-box-orient: vertical;
    overflow: hidden;
  }
}
</style>
