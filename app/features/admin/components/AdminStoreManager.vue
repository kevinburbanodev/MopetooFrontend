...
// Each row supports activate/deactivate and plan assignment
// via specific PATCH endpoints. No delete, verify, or featured toggles.
...
function planBadgeClass(plan: string): string {
  if (plan === 'featured') return 'bg-warning text-dark'
  return 'bg-secondary'
}
            ...
            <template v-else-if="adminStore.petshops.length > 0">
              <tr v-for="store in adminStore.petshops" :key="store.id">
                ...
                <td class="text-muted small">
                  ...
                  <span v-if="!store.email && !store.phone" class="text-muted">—</span>
                </td>
                ...
                <td class="text-end">
                  <div class="d-flex justify-content-end gap-1 flex-wrap">
                    ...
                    <button
                      type="button"
                      class="btn btn-sm"
                      :class="store.is_active ? 'btn-outline-secondary' : 'btn-outline-success'"
                      :aria-label="store.is_active ? `Desactivar ${store.name}` : `Activar ${store.name}`"
                      @click="store.is_active ? deactivateStore(store.id) : activateStore(store.id)"
                    >
